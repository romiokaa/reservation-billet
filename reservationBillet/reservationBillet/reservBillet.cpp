...
std::string Utilisateur::getNom() const { return nom; }

std::string Utilisateur::getPrenom() const { return prenom; }

std::string Utilisateur::getDateNaissance() const { return dateNaissance; }

std::string Utilisateur::getEmail() const { return email; }

int Utilisateur::getNumero() const { return numero; }

float Utilisateur::getSolde() const { return solde; }

//

void Utilisateur::setNom(std::string nouveauNom) { nom = nouveauNom; }

void Utilisateur::setPrenom(std::string nouveauPrenom) { prenom = nouveauPrenom; }

void Utilisateur::setDateNaissance(std::string nouveauDateNaissance) { dateNaissance = nouveauDateNaissance; }

void Utilisateur::setEmail(std::string nouveauEmail) { email = nouveauEmail; }

void Utilisateur::setNumero(int nouveauNumero) { numero = nouveauNumero; }

void Utilisateur::setSolde(float nouveauSolde) { solde = nouveauSolde; }
...
void GestionnaireReservations::reserverBillets(Utilisateur& u, Evenement e, int nbBillets) {
    for (Billet& billet : billets) {
        if (billet.nomEvenement == e.nom && billet.numeroUtilisateur == u.getNumero()) {
    ...
    for (Evenement& ev : evenements) {
        if (ev.nom == e.nom && ev.date == e.date) {
            ...
            if (u.getSolde() >= prixTotal) {
                ev.placesDisponibles -= nbBillets;
                u.setSolde(u.getSolde() - prixTotal);
                int numeroBillet = billets.size() + 1;
                Billet billet(numeroBillet, e.nom, nbBillets, prixTotal, u.getNumero());
...
void GestionnaireReservations::afficherBilletsUtilisateur(Utilisateur u) {
    for (const Billet& b : billets) {
        if (b.numeroUtilisateur == u.getNumero()) {
...
void Application::demarrer() {
    ...
    Utilisateur utilisateur = gestionnaire.trouverUtilisateur(user.getNom());
    gestionnaire.reserverBillets(utilisateur, eventChoisi, nbBillets);
    std::cout << "Mise à jour du solde : " << utilisateur.getSolde() << " $" << std::endl;
}
